...
class FixGeneratorAgent:
    ...
    def _generate_fix_for_bug(self, code: str, bug: Bug) -> Optional[Fix]:
        ...
        if bug.type == "ZeroDivisionError":
            # Fix division by zero issues - better implementation
            lines = code.split('\n')
            fixed_lines = []
            
            for line in lines:
                if bug.code_snippet.strip() in line.strip():
                    # Find the indentation level
                    indent = len(line) - len(line.lstrip())
                    spaces = ' ' * indent
                    
                    # Add the safety check
                    fixed_lines.append(f"{spaces}if not numbers:")
                    fixed_lines.append(f"{spaces}    return 0")
